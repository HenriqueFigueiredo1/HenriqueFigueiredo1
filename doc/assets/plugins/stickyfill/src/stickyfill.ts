 ...
 */
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'seppuku'.
let seppuku = false;
...
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'shadowRoot... Remove this comment to see the full error message
const shadowRootExists = typeof ShadowRoot !== 'undefined';
// Last saved scroll position
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'scroll'.
const scroll = {
    top: null,
    left: null
};
// Array of created Sticky instances
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'stickies'.
const stickies = [];
/*
 * 3. Utility functions
 */
 ...
 * 4. Sticky class
 */
// @ts-expect-error ts-migrate(2300) FIXME: Duplicate identifier 'Sticky'.
class Sticky {
    _active: any;
    _clone: any;
    _limits: any;
    ...
    constructor(node: any) {
        ...
        if (stickies.some((sticky: any) => sticky._node === node))
    ...
    refresh() {
         ...
         */
        if (isNaN(parseFloat(nodeComputedProps.top)) ||
    ...
    _recalcPosition() {
        if (!this._active || this._removed)
            return;
        const stickyMode = (scroll as any).top <= this._limits.start ? 'start' : (scroll as any).top >= this._limits.end ? 'end' : 'middle';
    ...
    _fastCheck() {
        if (!this._active || this._removed)
            return;
        if (Math.abs(getDocOffsetTop(this._clone.node) - this._clone.docOffsetTop) > 1 ||
    ...
    _deactivate() {
        ...
        if (!stickies.some((sticky: any) => sticky !== this && sticky._parent && sticky._parent.node === this._parent.node)) {
    ...
    remove() {
        this._deactivate();
        stickies.some((sticky: any, index: any) => {
 ...
 * 5. Stickyfill API
 */
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'Stickyfill... Remove this comment to see the full error message
const Stickyfill = {
    stickies,
    Sticky,
    addOne(node: any) {
    ...
    refreshAll() {
        stickies.forEach((sticky: any) => sticky.refresh());
    },
    removeOne(node: any) {
        // Check whether it’s a node
        if (!(node instanceof HTMLElement)) {
...
function init() {
    ...
    function checkScroll() {
        if (window.pageXOffset != (scroll as any).left) {
            (scroll as any).top = window.pageYOffset;
            (scroll as any).left = window.pageXOffset;
            Stickyfill.refreshAll();
        }
        else if (window.pageYOffset != (scroll as any).top) {
            (scroll as any).top = window.pageYOffset;
            (scroll as any).left = window.pageXOffset;
            // recalc position for all stickies
            stickies.forEach((sticky: any) => sticky._recalcPosition());
        }
    }
    checkScroll();
    ...
    function startFastCheckTimer() {
        fastCheckTimer = setInterval(function () {
            stickies.forEach((sticky: any) => sticky._fastCheck());
        }, 500);
    }
    function stopFastCheckTimer() {
        clearInterval(fastCheckTimer);
    }
    let docHiddenKey: any;
